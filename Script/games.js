...
`Lavender Haze`, `Maroon`, `Anti-Hero`, `Snow On The Beach`, `You're On Your Own, Kid`, `Midnight Rain`, `Question...?`, `Vigilante Shit`, `Bejeweled`, `Labyrinth`, `Karma`, `Sweet Nothing`, `Mastermind`, `The Great War`, `Bigger Than The Whole Sky`, `Paris`, `High Infidelity`, `Glitch`, `Would've, Could've, Should've`, `Dear Reader`, `Hits Different`, `Snow On The Beach (Featuring More Lana Del Ray)`, `Karma (Ice Spice Remix)`, `You're Losing Me`
];
// Define an array of options | Albums
const albumOptions = [
'Taylor Swift (Debut).<br><a href="/taylor-swift"><img src="Images/Album-Covers/ts1.webp" alt="Taylor Swift Album Cover"></a>',
'Fearless.<br><a href="/fearless"><img src="Images/Album-Covers/ts2.webp" alt="Fearless Album Cover"></a>',
'Speak Now.<br><a href="/speak-now"><img src="Images/Album-Covers/ts3.webp" alt="Speak Now Album Cover"></a>',
'Red.<br><a href="/red"><img src="Images/Album-Covers/ts4.webp" alt="Red Album Cover"></a>',
'1989.<br><a href="/1989"><img src="Images/Album-Covers/ts5.webp" alt="1989 Album Cover"></a>',
'reputation.<br><a href="/reputation"><img src="Images/Album-Covers/ts6.webp" alt="reputation Album Cover"></a>',
'Lover.<br><a href="/lover"><img src="Images/Album-Covers/ts7.webp" alt="Lover Album Cover"></a>',
'folklore.<br><a href="/folklore"><img src="Images/Album-Covers/ts8.webp" alt="folklore Album Cover"></a>',
'evermore.<br><a href="/evermore"><img src="Images/Album-Covers/ts9.webp" alt="evermore Album Cover"></a>',
'Midnights.<br><a href="/midnights"><img src="Images/Album-Covers/ts10.webp" alt="Midnights Album Cover"></a>',
];
...
albumGenerateBtn.addEventListener('click', function() {
    ...
    // Update the content of the result element
		...
		`Stumble down pretend alleys`,
		`Waste cheap wine, make believe it's champagne`,
		...
		`Flexing like a goddamn acrobat`,
		`Me and karma vibe like that`,
		`Karma is ya check's 'boutta bounce (Damn)`,
		`Karma is the fire in your house (Grah)`,
		...
		`I be draggin' that wagon, karma is a beauty winning that pageant (Grah)`,
...
const songs = [
 ...
 },
 {
   ...
   {
      title: `Miss Americana & Heartbreak Prince`,
      album: `Lover`,
      lyrics: [
         `You know I adore you, I'm crazier for you`,
         `Than I was at 16, lost in a film scene`,
   ...
   {
      title: `Snow On The Beach (Featuring More Lana Del Ray)`,
      album: `Midnights`,
      lyrics: [
         `One night, a few moons ago`,
         `I saw flecks of what could've been lights`,
 ...
 },
 {
	...
	lyrics: [
		...
		`Make a ni**a wanna run straight through the fence`,
...
// Add event listener for clicking outside the input field
...
function search() {
   const keyword = searchInput.value.toLowerCase().replace(/[^\w\s\d]/g, '');
   const results = songs.filter(song => {
      if (typeof song.lyrics === 'string') {
      const lyricsWithoutPunctuation = song.lyrics.toLowerCase().replace(/[^\w\s\d]/g, '');
      return lyricsWithoutPunctuation.includes(keyword);
      } else if (Array.isArray(song.lyrics)) {
      return song.lyrics.some(line => {
         const lineWithoutPunctuation = line.toLowerCase().replace(/[^\w\s\d]/g, '');
         return lineWithoutPunctuation.includes(keyword);
      });
      }
      return false;
   });
...
function highlightKeyword(text, keyword) {
  const regex = new RegExp(`(${keyword.replace(/[^\w\s\d]/g, '')})`, 'gi'); // Remove punctuation from keyword for matching
...
function displayResults(results, keyword) {
   ...
   if (results.length === 0) {
      ...
      album.innerHTML = `<span class="album-title">${results[i].album}</span>`;
      album.classList.add(`album-${results[i].album.toLowerCase().replace(/\s+/g, '-')}`);
      result.appendChild(title);
      result.appendChild(album);
      result.appendChild(lyrics);

      let matchedLines = '';
      if (typeof results[i].lyrics === 'string') {
         const lyricsWithoutPunctuation = results[i].lyrics.toLowerCase().replace(/[^\w\s\d]/g, '');
         matchedLines = highlightKeyword(lyricsWithoutPunctuation, keyword);
      } else if (Array.isArray(results[i].lyrics)) {
         const matchingLines = results[i].lyrics.filter(line => {
            const lineWithoutPunctuation = line.toLowerCase().replace(/[^\w\s\d]/g, '');
            return lineWithoutPunctuation.includes(keyword);
         });
         matchedLines = matchingLines.map(line => highlightKeyword(line, keyword)).join('<br>');
      }
      lyrics.innerHTML = matchedLines;
